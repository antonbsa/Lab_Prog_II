...
import TrabMod13_Arvores.BSTNode;
...
public class BST {
  ...
  **/
  ...
  private void print(String prefix, BSTNode n, boolean isLeft) {
  	if(n != null) {
  ...
  // Métodos de implementação do módulo 13
  	...
  	public int countLeafNodes() {
  		if(root == null)
  			return 0;
  		else return countLeafNodes(root);
  	}
  	
  	protected int countLeafNodes(BSTNode p) {
  		
  		if(p.left != null && p.right != null)
  			return 0 + countLeafNodes(p.left) + countLeafNodes(p.right);
  		else return 1;
  	...
  	protected boolean isFull(BSTNode p){
  		int result = (int) Math.pow(2 ,height()) - 1;
  		
  		if(result == countNodes())
  			return true;
  		else
  			return false;
  	}
